...
class Transformer(nn.Module):
    ...
    def decoder(self, encoder_output, src_mask, trg, trg_mask):
...
def build_transformer(src_vocab_size: int, trg_vocab_size: int, src_seq_len: int, trg_seq_len: int, d_model: int = 512, N: int = 6, h: int = 8, dropout: float = 0.1, d_ff: int = 2048) -> Transformer:
    ...
    for _ in range(N):
        ...
        feed_forward_block = FeedForwardBlock(d_model, d_ff, dropout)
        decoder_block = Decoder(decoder_self_attention_block, decoder_cross_attention_block, feed_forward_block, dropout)
